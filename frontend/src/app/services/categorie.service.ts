...
return this.http.post<Categorie>(this.apiUrl, donnees).pipe(
  tap((categorie) => {
...
return this.http.put<Categorie>(`${this.apiUrl}/${id}`, donnees).pipe(
  tap((categorieModifiee) => {
    this.categoriesSignal.update((categories) =>
      categories
        .map((categorie) =>
          categorie.id === id ? categorieModifiee : categorie,
        )
        .sort(
          (a, b) =>
            a.ordreAffichage - b.ordreAffichage ||
...
return this.http.delete<void>(`${this.apiUrl}/${id}/definitif`).pipe(
  tap(() => {
    this.categoriesSignal.update((categories) =>
      categories.filter((categorie) => categorie.id !== id),
    );
