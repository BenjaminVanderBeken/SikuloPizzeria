...
chargerParProduit(
produitId: number,
): Observable<CompositionProduit[]> {
this.chargement.set(true);
this.erreur.set('');


return this.http
  .get<CompositionProduit[]>(
  ...
  .pipe(
    tap({
      ...
      error: () => {
        this.erreur.set(
          'Impossible de charger la composition du produit.',
        );
        this.chargement.set(false);
      },
    }),
...
creer(
dto: CreateCompositionProduitDto,
): Observable<CompositionProduit> {
this.chargement.set(true);
this.erreur.set('');
    ...
    error: (erreur) => {
      this.erreur.set(
        erreur.error?.message ??
          'Impossible de créer la composition.',
      );
      this.chargement.set(false);
    },
  }),
...
return this.http.delete<void>(`${this.apiUrl}/${id}`).pipe(
  tap({
    ...
    error: () => {
      this.erreur.set(
        'Impossible de supprimer cette composition.',
      );
      this.chargement.set(false);
    },
  }),
