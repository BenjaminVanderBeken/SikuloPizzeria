...
export class ProduitsPage implements OnInit {
  ...
  readonly formulaire = this.formBuilder.group({
    ...
    prixPromotion: [null as number | null, [Validators.min(0)]],
    imageUrl: [''],
  ...
  enregistrer(): void {
    ...
    const dtoCreation: CreateProduitDto = {
      ...
      description: this.nettoyerTexte(valeur.description),
      prixBase: Number(valeur.prixBase),
      prixPromotion:
        valeur.prixPromotion === null
          ? null
          : Number(valeur.prixPromotion),
      imageUrl: this.nettoyerTexte(valeur.imageUrl),
      permetSupplement: Boolean(valeur.permetSupplement),
      populaire: Boolean(valeur.populaire),
      ordreAffichage: Number(valeur.ordreAffichage),
      variantes,
    };

    const produitId = this.produitService.produitEnEditionId();

    if (produitId === null) {
      this.produitService.creer(dtoCreation).subscribe({
    ...
    this.produitService.modifier(produitId, dtoModification).subscribe({
      next: () => this.reinitialiserFormulaire(),
      error: () => undefined,
    });
  }
  ...
  modifier(produit: Produit): void {
    this.produitService.selectionnerPourEdition(produit.id);
    ...
    this.formulaire.patchValue({
      ...
      ordreAffichage: produit.ordreAffichage,
    });
  ...
  desactiver(produit: Produit): void {
    ...
    this.produitService.desactiver(produit.id).subscribe({
      error: () => undefined,
    });
  }
