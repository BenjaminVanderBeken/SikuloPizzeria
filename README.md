...
## 15. Nettoyage avant creation du ZIP

Avant de creer le ZIP de remise, il faut exclure les dossiers generes automatiquement.
...
Commande possible depuis la racine :

```
Get-ChildItem . -Recurse -Directory -Include bin,obj,dist,node_modules,.vs |
Remove-Item -Recurse -Force
```

Attention : apres suppression de node_modules, il faudra refaire npm install dans le dossier frontend pour relancer l'application.

---

## 16. Contenu attendu dans le ZIP
...
Le fichier ZIP ne doit pas contenir :
...
git status
git add .
git commit -m "Finalisation du projet SikuloPizzeria"
git push
```

Avant le commit, verifier que le dossier `frontend/public/images` est bien inclus afin que le logo de l'application soit disponible sur GitHub.

---

## 18. Objet de l'e-mail de remise

L'e-mail envoye a l'enseignant doit avoir comme objet :

```
[Angular/.NET] Remise projet - Vander Beken Benjamin
```

Le contenu de l'e-mail doit contenir le lien vers le depot GitHub public.

---

## 19. Points importants respectes

Le projet respecte les consignes suivantes :

* Backend en ASP.NET Core
* Solution decoupee en trois projets : API, Core et Infrastructure
* Clean Architecture avec separation claire des responsabilites
* Acces a la base de donnees uniquement avec Dapper
* Utilisation du pattern Repository
* Absence d'Entity Framework
* Aucun acces SQL direct depuis les controleurs
* Separation claire des responsabilites
* Frontend Angular
* Utilisation de composants Angular
* Utilisation du routage Angular
* Utilisation des services Angular
* Utilisation de Reactive Forms
* Communication avec l'API via HttpClient
* Utilisation de la nouvelle syntaxe Angular @if et @for
* Gestion de l'etat uniquement dans les services Angular
* Absence de NgRx et Redux
* Base de donnees MySQL fonctionnelle
* Scripts SQL fournis
* README present a la racine du projet

---

## 20. Resume oral

SikuloPizzeria est une application de gestion pour une pizzeria.

Le frontend est developpe avec Angular. Les pages Angular affichent les donnees et utilisent des services dedies pour communiquer avec l'API via HttpClient. L'etat de l'application est gere dans les services Angular avec des signals.

Le backend est developpe avec ASP.NET Core et organise en trois couches principales :
...
Le projet API contient les controleurs.

Le projet Core contient les entites, les DTOs, les interfaces et les services metier.

Le projet Infrastructure contient les repositories Dapper et les requetes SQL.

Lorsqu'une donnee est demandee depuis l'interface, le flux est le suivant :
...
Le retour de la donnee suit le chemin inverse jusqu'a l'affichage dans le composant Angular.

Aucune requete SQL n'est placee dans les controleurs.

L'acces a la base de donnees passe uniquement par les repositories avec Dapper.

Entity Framework n'est pas utilise dans le projet.
